...
#### 问题
* 衰减任务时如果处理读请求? 如果一个缓存在本次将要被删除时，刚好又来了个查询该如何处理?

  * 执行衰减任务在副本zset上进行（ZUNIONSTORE new_zset 1 source_zset），完成后通过rename覆盖 
  * 将要被删除的key会给一个5分钟的ttl，衰减任务执行完后还可以对其添加score，使其恢复
